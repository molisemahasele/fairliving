...
$select = mysqli_query($conn, "SELECT * FROM contacts ORDER BY id ASC LIMIT " . $this_page_first_result . ',' . $results_per_page );
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card">';
                    echo '<div class="card-body">
                          <h3>'.$row['fullname'].'</h3>
                        <p class="card-text">'.$row['info'].'<br> landlord: <b>'.$row['landlord_first'].' '.$row['landlord_last'].'</b><br>id: <b>'.$row['postid'].'</b></p>
                        <p>'.$row['date'].'<br> <a href="delete.php?id='.$row['id'].'" class="btn btn-outline-danger">delete</a> 
                        <a href="reply.php?id='.$row['userId'].'" class="btn btn-outline-primary">reply</a>
                        </p>
                    </div>
                </div>
            </div>';
