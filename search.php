...
{
	$search = $_POST['search'];
	$sql = mysqli_query($conn, "SELECT * FROM posts WHERE location LIKE '%$search%' OR houseType LIKE '%$search%' OR firstname LIKE '%$search%' OR lastname LIKE '%$search%' OR info LIKE '%$search%'");
	$querySql = mysqli_num_rows($sql);

	if($querySql > 0)
	{
        echo '<div class="container">';
        echo "<b>There are ". $querySql. " results!</b><br>";
